...
- **지난 점검 기록** — 목록에서 항목을 열어 상세의 textarea로 조회·수정·클립보드 복사. 목록 행에서는 복사하지 않음(데이터 유실 방지를 위해 상세 진입 후 명시 복사). 헤더의 **"선택"** 버튼으로 다중 선택 모드 진입 → 항목 탭으로 토글 → 하단의 **"선택한 N개 휴지통으로 보내기"** 버튼으로 한 번에 휴지통으로 이동. (**예외 동선** — 일상 점검에는 다중 선택을 두지 않고, 학기 말 정리 같은 관리 작업에 한해 허용.)
- **휴지통(2단계 삭제 + 미리보기)** — "삭제"는 즉시 영구 삭제가 아니라 **휴지통으로 이동**(소프트 삭제). 지난 점검 목록 하단의 **"휴지통 (N) →"** 진입점에서 보관 중인 점검 기록을 보고, 항목별 **[복구]** 또는 **[영구 삭제]** 중 하나를 골라야 영구 삭제가 일어난다. 항목 상단을 탭하면 detail 화면으로 들어가 **본문 내용을 읽고**(편집은 잠금, 복사는 가능) 거기서도 복구·영구 삭제할 수 있어 영구 삭제 직전에 안전하게 내용을 확인할 수 있다. 본문은 휴지통에 있는 동안 그대로 보존되어 실수로 삭제해도 되돌릴 수 있다.
...
- **오프라인 안내 띠(전 화면 노출)** — 네트워크가 끊기면 모든 화면(홈·점검 진행·지난 점검 목록·상세) 최상단에 얇은 띠가 뜬다("오프라인이에요. 기록은 이 기기에 그대로 저장돼요"). 본 앱은 오프라인에서도 정상 동작하므로 경고가 아닌 안심 톤이며, 헤더와 독립적으로 동작해 비홈 화면(헤더가 없는 화면)에서는 `topmost` 모드로 노치 safe-area를 직접 흡수한다.
...
- **백업 추적(복사한 적 있음/신규)** — 각 세션이 한 번이라도 복사됐는지를 본문과 분리된 메타 네임스페이스(`book-scanner:meta:`)에 별도 저장한다. 1·2차 안전망(진행 중 복사 / 종료 후 상세 자동 진입 + 큰 복사 CTA)의 실제 성공 시 자동으로 "복사됨" 처리. 지난 점검 목록에서 권수>0 세션에 **"✓ 복사한 적 있음"**(목련 그린, 완료 톤) 또는 **"● 신규"**(연꽃 로즈, 강조 톤) 작은 뱃지로 구분되어, 학생도 사서 선생님도 어느 점검이 아직 전달 안 됐는지 한눈에 본다. 종료 직후 자동 진입 배너는 이미 복사된 세션이면 노출되지 않아 중복 안내가 없다.
