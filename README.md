...
- **지난 점검 기록** — 목록에서 항목을 열어 상세의 textarea로 조회·수정·클립보드 복사. 목록 행에서는 복사하지 않음(데이터 유실 방지를 위해 상세 진입 후 명시 복사). 헤더의 **"선택"** 버튼으로 다중 선택 모드 진입 → 항목 탭으로 토글 → 하단의 **"선택한 N개 휴지통으로 보내기"** 버튼으로 한 번에 휴지통으로 이동.
