...
- **백업 추적(복사한 적 있음/신규)** — 각 세션이 한 번이라도 복사됐는지를 본문과 분리된 메타 네임스페이스(`book-scanner:meta:`)에 별도 저장한다. 1·2차 안전망의 실제 성공 시 자동으로 "복사됨" 처리. 지난 점검 목록에서 권수>0 세션에 **"✓ 복사한 적 있음"**(목련 그린, 완료 톤) 또는 **"● 신규"**(연꽃 로즈, 강조 톤) 작은 뱃지로 구분되어, 학생도 사서 선생님도 어느 점검이 아직 전달 안 됐는지 한눈에 본다. 종료 직후 자동 진입 배너는 이미 복사된 세션이면 노출되지 않아 중복 안내가 없다.
